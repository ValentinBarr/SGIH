(() => {
  ...
  $('#frm-out').addEventListener('submit', async (e)=>{
    e.preventDefault();
    const depId = depSel.value;
    if(!depId) return;
    ...
    const r = await fetch('/inventarios/stock/salida', {
      method: 'POST', headers: {'Content-Type':'application/json'},
      body: JSON.stringify({ depId: Number(depId), items, nota: 'Salida desde formulario' })
    });
    const data = await r.json();
