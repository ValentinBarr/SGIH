...
router.get('/compras/remitos', async (req, res) => {
  try {
    ...
    res.send(remitosTemplate({ remitos, proveedores, estados, filters: { q: q || '', id_prov: id_prov || '', estado: estado || '' }, basePath: '/compras/remitos' }));
  } catch (error) {
...
router.get('/compras/remitos/new', async (req, res) => {
  try {
    ...
    const estados = RemitosRepo.getEstadosDisponibles();
    res.send(renderForm({ mode: 'new', proveedores, productos, estados, remito: { fecha: new Date().toISOString().split('T')[0], detalles: [], estado: ESTADOS_REMITO.BORRADOR } }));
  } catch (error) {
...
router.get('/compras/remitos/:id/edit', async (req, res) => {
  try {
    ...
    const proveedores = await RemitosRepo.getActiveProveedores();
    const depositos = await RemitosRepo.getActiveDepositos();
    const productos = await RemitosRepo.getActiveProductos();
    const estados = RemitosRepo.getEstadosDisponibles();
    res.send(renderForm({ mode: 'edit', remito, proveedores, depositos, productos, estados }));
  } catch (error) {
...
router.post('/compras/remitos/new', async (req, res) => {
  try {
    ...
    if (Object.keys(errors).length > 0) {
      ...
      const productos = await RemitosRepo.getActiveProductos();
      const estados = RemitosRepo.getEstadosDisponibles();
      return res.status(400).send(renderForm({ mode: 'new', proveedores, productos, estados, remito: { ...body, detalles }, errors }));
    }
...
router.post('/compras/remitos/:id/edit', async (req, res) => {
  try {
    ...
    const updateData = {
      ...
      id_dep: Number(body.id_dep),
      letra_comp: (body.letra_comp || 'R').toUpperCase(),
      sucursal_comp: (body.sucursal_comp || '0000').padStart(4, '0'),
      numero_comp: (body.numero_comp || '0').padStart(8, '0'),
      observacion: body.observacion || null,
      estado: body.estado,
      detalles
...
router.post('/compras/remitos/:id/estado', async (req, res) => {
    // (Este código es de una versión anterior tuya, lo mantengo por si lo necesitas)
    const id = Number(req.params.id);
